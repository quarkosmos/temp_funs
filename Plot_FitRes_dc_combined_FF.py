...
class PlotFitRes(Cube2map):
    ...
    def  __init__(self, cube=None, ext=None, region='SerB_', getrms='both', rmssize=300, max_rms=None,
    ...
    def plot_fof_profile(self, tdet=None, line='C18O', vr=None, yr=None, n_ch=None, nzp=None, vtype=None, nbeam=None, dckey=None, \
                          filsigmacut = None, dv0 = None, drd0 = None, fof_result = None, dcresult=None, dctype=' ', note='', fntype=None):
        """
        Interactive scan for fitting results.
        Plot fitting results over line profile on click position.
        Parameters:
...
'darkslateblue', 'mediumslateblue', 'mediumvioletred', 'chartreuse', 'burlywood']
        ...
        coloridx = np.zeros(len(tdet[ifn]))
        cindex = 0
        a = 0
        filzero_check = f_count_sort[0][0]
        for ifil in range(np.shape(f_count_sort)[0]):
            ...
            if filzero_check == 0:
                ...
                cindex = ifil+1
            ctemp_index = np.where(tdet[ifn]==fil)
            coloridx[ctemp_index] = cindex

            if cnt > npix0:
                fof_result = tdet[(tdet[ifn] == fil)]
                ...
                lsmap.scatter(fof_result['rp'], fof_result['dp'], marker='s', s=5, alpha=0.2, color = clr_list[cindex])
