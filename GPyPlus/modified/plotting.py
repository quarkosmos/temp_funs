...
# Modification by KHKim since 2021-07-14
# plot_spectra: add fig_min_chan, fig_max_chan to allow user select velocity range to plot
...
def plot_spectra(pathToDataPickle, *args,
    ...
    fig_min_channel, fig_max_channel = fig_channels[fig_min_chan], fig_channels[fig_max_chan]
    if fig_max_chan != -1:
        n_channels = (fig_max_chan - fig_min_chan)+1
    else:
        n_channels = len(channels)
    ...
    fig_channels_cut = fig_channels[fig_min_chan:fig_max_chan+1] # added by KHK
#print(fig_channels)
...
#ax.step(fig_channels, y, color='black', lw=0.5, where='mid')
        ax.step(fig_channels[fig_min_chan:fig_max_chan+1], y[fig_min_chan:fig_max_chan+1], color='black', lw=0.5, where='mid')
        ...
        if decomposition or training_set:
            ...
            ax.plot(fig_channels[fig_min_chan:fig_max_chan+1], combined_gauss[fig_min_chan:fig_max_chan+1], lw=2, color='orangered') #modified by KHK
            ...
            if gaussians:
                for j in range(ncomps):
                    gauss = gaussian(fit_amps[j], fit_fwhms[j], fit_means[j], channels)
                    ax.plot(fig_channels[fig_min_chan:fig_max_chan+1], gauss[fig_min_chan:fig_max_chan+1], ls='solid', lw=1, color='orangered')
        ...
        if residual and (decomposition or training_set):
            ...
            ax.step(fig_channels[fig_min_chan:fig_max_chan+1], y[fig_min_chan:fig_max_chan+1] - combined_gauss[fig_min_chan:fig_max_chan+1], color='black', lw=0.5, where='mid')
            if signal_ranges:
                plot_signal_ranges(ax, data, idx, fig_channels)
...
#fig = plt.figure(figsize=figsize, constrained_layout=True)
            ...
            gridspec.GridSpec(cols, remaining_rows, wspace=0.1, hspace=0.5)
